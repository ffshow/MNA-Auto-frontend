...
class OwnerService {
  ...
  Stream<OwnerModel> get onCreate => createStream.stream;

  StreamController<OwnerModel> updateStream = StreamController.broadcast();
  Stream<OwnerModel> get onUpdate => updateStream.stream;

  StreamController<OwnerModel> deleteStream = StreamController.broadcast();
  Stream<OwnerModel> get onDelete => deleteStream.stream;

  void create(OwnerModel g) {
    createStream.sink.add(g);
  }

  void delete(OwnerModel g) {
    deleteStream.sink.add(g);
  }

  void update(OwnerModel g) {
  ...
  ///
  /// Path /api/owner_list
  Future<ListOwnerModel> getApiOwnerList({
    int? page,
    int? per_page,
    String? sort_by,
    bool? descending,
    ...
    if (response.statusCode == 200) {
      return ListOwnerModel.fromJson(response.data as Map<String, Object?>);
    }
